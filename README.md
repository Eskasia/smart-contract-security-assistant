...
- 靜態分析：Slither detector 映射 reentrancy、access control、unchecked external call、delegatecall、controlled array length、oracle、price manipulation、privilege escalation、upgrade risk。
...
- Trace——SQLite 追蹤每個 finding 的 Slither raw output、標準化結果、RAG chunk、prompt、LLM output、judge score、token usage 與 review status。
- Report——Markdown/JSON 直接輸出 vulnerable code snippet、自然語言 explanation、attack path、fix suggestion、AI remediation code、local/external judge score 與 prompt/completion/total tokens。

## 驗證狀態

2026-05-04 驗證通過：`uv run pytest` 共 27 passed，`uv run ruff check .` 通過，`npm run test` 共 5 passed，`npm run build` 通過。2026-05-01 驗證通過：`uv run python eval/run_eval.py` 召回率 1.0，`uv run python eval/run_judge.py` local/external 平均分皆 5.0，Web50 corpus 為 `unknown_rate=0.1916`、`eligible_chunks=637`；`VulnerableVault.sol` 實測報告輸出 prompt/completion/total tokens 為 `680/300/980`，local/external judge score 皆 `5.00/5`。CI 已接入 ruff、pytest、RAG recall eval、judge eval。
