...
- 2026-06-01 已新增 API fail-closed：`/api/*` 預設要求 bearer token，非本機 host 未提供 `--api-token` 時拒絕啟動，tokenless local demo 與任意 input root 都必須顯式 opt in，mismatched Origin 與非 JSON write body 會被拒絕，token 模式拒絕 wildcard CORS，並加入 max concurrent jobs、event buffer 與 report read size 上限。
...
HTTP API——本機 stdlib `ThreadingHTTPServer`，加固入口命令為 `uv run scsa api --host 127.0.0.1 --port 8787 --out-dir reports-api --input-root "$PWD" --api-token dev-token --cors-origin http://127.0.0.1:5173 --max-request-bytes 1048576 --max-concurrent-jobs 4 --max-events-per-job 256 --max-report-bytes 5000000 --native-build-policy disabled`；支援 analysis job、SSE stream、JSON report、Markdown report download、SQLite trace lookup、整份 report review status 與逐條 finding review 寫回。`/api/*` 預設要求 bearer token；tokenless local demo 只能用 `--allow-tokenless-local-demo` 顯式開啟，沒有 `--input-root` 的任意 path analysis 只能用 `--allow-any-input-root` 顯式開啟；外部 host 必須提供 bearer token，token 模式必須使用固定 CORS origin。
