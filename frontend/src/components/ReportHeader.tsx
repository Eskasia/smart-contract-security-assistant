...
export function ReportHeader() {
  ...
  return (
    <header className="border-b border-border-subtle bg-surface px-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge status={report.overall_status} />
            <ReviewBadge status={report.review_status} />
            <span className="rounded-sm border border-border-subtle bg-surface-muted px-2 py-0.5 text-xs font-medium text-text">
              {connectionMode.toUpperCase()}
            </span>
          </div>
          <h1 className="mt-2 text-xl font-semibold tracking-normal text-text-strong">
            {t("appTitle")}
          </h1>
      ...
      <div className="mt-4 grid gap-3 border-t border-border-subtle pt-3 text-sm text-text md:grid-cols-4">
